...
(function () {
  ...
  function categoriasDoPrograma() {
    ...
    const m = new Map();
    for (const p of prog) {
      // categoria com "+" já é junção pronta: o que serve de peça aqui são
      // as partes dela, que é como as categorias vêm escritas nos inscritos
      for (const parte of partesVisiveis(p.rotulo || p.categoria)) {
        if (parte) m.set(D.chaveCategoria(parte), parte);
      }
    }
    return [...m.values()];
  }

  // a letra no fim, com ou sem aspas, precedida de espaço: PARALÍMPICO "A"
  const LETRA_NO_FIM = /^(.*?)\s+("?[A-Za-zÀ-ÿ]"?)$/;

  /* 'PARALÍMPICO "A" + "B"' vira ['PARALÍMPICO "A"', 'PARALÍMPICO "B"'],
     mantendo o jeito que o programa escreveu. A parte que é só a letra herda
     a palavra da primeira. */
  function partesVisiveis(categoria) {
    const t = String(categoria == null ? "" : categoria).trim();
    if (t.indexOf("+") < 0) return [t];
    const cruas = t.split("+").map((x) => x.trim()).filter(Boolean);
    const m = cruas[0].match(LETRA_NO_FIM);
    const base = m ? m[1].trim() : "";
    return cruas.map((p, i) => {
      if (i === 0) return p;
      return base && /^"?[A-Za-zÀ-ÿ]"?$/.test(p) ? base + " " + p : p;
    });
  }

  /* O nome da prova que sai de um agrupamento. Quando as categorias só se
     diferenciam pela letra, sai no formato do programa, PARALÍMPICO "A" + "B",
     e por isso volta a casar com a prova que já existe lá. */
  function nomeDoGrupo(cats) {
    const c = (cats || []).filter(Boolean);
    if (c.length < 2) return c[0] || "";
    const pedacos = c.map((x) => x.match(LETRA_NO_FIM));
    if (pedacos.every(Boolean)) {
      const base = pedacos[0][1].trim();
      if (base && pedacos.every((m) => m[1].trim() === base)) {
        return base + " " + pedacos.map((m) => m[2]).join(" + ");
      }
    }
    return c.join(" + ");
  }
  ...
  function provasDoGrupo(cats) {
    ...
    // a prova do programa entra se qualquer parte dela for uma das escolhidas:
    // quem escolhe PARALÍMPICO "A" está mirando a prova A + B também
    return (estado.perfil.programa || []).filter((p) =>
      D.partesDaCategoria(p.categoria).some((parte) => chaves.includes(parte)));
  }
  ...
  function linhaDeGrupo(g, k) {
    ...
    // quantas caixas mostrar é escolha do árbitro, não do que ele já
    // preencheu: senão nunca dava para subir de duas para três
    const quantas = Math.max(2, g.quantas || cats.length || 2);
    ...
    const atualizarSaida = () => {
      ...
      alvo.innerHTML = `Vai sair como <b>${AT(nomeDoGrupo(c))}</b>${onde}${
    ...
    $$("select", cartao).forEach((sel) => {
      sel.onchange = () => {
        if (sel.dataset.c === "quantas") {
          g.quantas = parseInt(sel.value, 10) || 2;
          g.categorias = escolhidas().slice(0, g.quantas);
          g.rotulo = nomeDoGrupo(g.categorias);
          renderGrupos();
          return;
        }
        guardar();
